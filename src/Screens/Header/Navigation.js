import { Badge, CardContent, Container, CssBaseline, Divider, Grid, ListItemIcon, Menu, MenuItem, Paper, Tooltip } from "@material-ui/core";
...
const Navigation = () => {
  ...
  const globalsnackbar = useSelector((state) => state.DefaultReducers.globalsnackbar);
  ...
  useEffect(() => {
    let mounted=true
      const index=async ()=>{
            ...
            getpermissions?.map(async(item)=>{
              if(item?.logid==="approve"){
                  approve=true
              }else if (item?.logid==="cancel"){
                  cancel=true
              }
           
         
              return await dispatch(set_permissions(approve,cancel))
            })
  ...
  }, [dispatch, offset, user_info?.deptcode, username]);
  ...
  useEffect(() => {
    ...
    return () => (mounted = false);
  }, [IsRunning, calling, dispatch, hubconnectnotify, play]);
  ...
  const [NotificationsMobileAnchorEl, setNotificationsMobileAnchorEl] = React.useState(null);
...
setnotificationopen(false)
  ...
  const renderNotifList = (
   ...
   open={isMenuNotificationsOpen}
      ...
      return(
        <div>
        <MenuItem onClick={()=>HandleopenNotification(item)} key={index}>
        <ListItemIcon>
          ...
          <Typography variant="inherit" >{item.createdBy} : {item?.body}</Typography>
      ...
      <div style={{marginTop:100}}>
         <CustomModal
            ...
            UI={
              ...
              <Grid item xs={12}>
              <Typography style={{textAlign:"center",fontSize:"16",fontWeight:800,marginTop:50}}>
                 { notificationtitle}
            </Typography>
            <Typography  style={{textAlign:"center",fontSize:"12",fontWeight:500}}>
                 { notificationbody}
            </Typography>
            <div style={{display: 'flex',  justifyContent:'center', alignItems:'center',marginTop:50}}>
            <Button    variant="contained" color="primary" onClick={()=>handleNotificationModalClose()}>
           Close
        </Button> 
        </div>
              </Grid>

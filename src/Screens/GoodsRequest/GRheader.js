...
import { Button, CardContent, Container, Grid, Paper, Typography } from '@material-ui/core';
import TextField from '@material-ui/core/TextField';
import FormControl from '@material-ui/core/FormControl';
import Select from '@material-ui/core/Select';
import InputLabel from '@material-ui/core/InputLabel';
import MenuItem from '@material-ui/core/MenuItem';
import {action_set_office,action_set_openmodal} from '../../Services/Actions/Inventory_Actions';
import useStyles from './style'
import {set_global_sncakbar,action_INSERT_notications} from '../../Services/Actions/Defaults_Actions'
import {action_set_notification,action_set_requestedheade,action_set_requestedfooter,action_GET_InsertNewRequest,action_close_success_modal,action_SET_updaterequestApproved,action_SET_updaterequestCancelled} from '../../Services/Actions/Inventory_Actions'
...
const GRheader = props => {
    ...
    const department = useSelector((state) => state.Inventory_Reducers.department);
    const requesteddtls = useSelector((state) => state.Inventory_Reducers.requesteddtls);
    const requestsuccess = useSelector((state) => state.Inventory_Reducers.requestsuccess);
    const singlerequestheader = useSelector((state) => state.Inventory_Reducers.singlerequestheader);
    ...
    const permissions = useSelector(
      (state) => state.DefaultReducers.permissions
    );
    ...
    useEffect(()=>{
      ...
      const checkifrequestshow=async()=>{
        if (mounted){
  if(requestinfo?.data.STATUS==="Approved" || requestinfo?.data.STATUS==="Cancelled"){
  ...
  dispatch(action_set_requestedfooter(singlerequestheader?.data[0]?.reqremarks))
}else if(singlerequestheader?.loading && requestinfo?.data?.reqno!==undefined ){
...
if(permissions?.approve && permissions?.cancel){
  ...
  setcancancel(false)
    ...
    },[dispatch, reqstatus, requestinfo, singlerequestheader, singlerequestheader?.data.reqno, singlerequestheader?.loading])
    ...
    const handleSubmitRequest= useCallback(async()=>{
        ...
        if (mounted){
          if(requesteddtls!==[])
           await dispatch(action_GET_InsertNewRequest(user_info?.deptcode,reqby,office,reqremarks,requesteddtls))
  ...
  const successgetter=async()=>{
    if (mounted){
      if(requestsuccess?.loading)
         ...
         dispatch(action_set_notification(true,requestsuccess?.message,"info","System",office))
...
let mounted=true
if (mounted){
dispatch(action_SET_updaterequestApproved(reqno,user_info?.username,user_info?.empname))
}
return()=>{mounted=false}
...
let mounted=true
if (mounted){
dispatch(action_SET_updaterequestCancelled(reqno,user_info?.username,user_info?.empname))
}
return()=>{mounted=false}
},[dispatch, reqno, user_info?.empname, user_info?.username])
  ...
  history.push('/Maintable')
    ...
    return (
        ...
        <div style={{marginLeft:"auto"}}>
          {canapprove? <Button   style={{marginRight:20}} variant="contained" color="primary" onClick={()=>handleApproveRequest()}>
           Approve
        </Button> :null}
        {cancancel? <Button   variant="contained" color="secondary" onClick={()=>handleCancelRequest()}>
           Cancel
        </Button> :null}
        ...
        :
        <>
        <Button  style={{ marginRight:"auto" }} disabled={isDisabled} variant="contained" color="primary" onClick={()=>handleNewItemClick()}>
           Add Item
        </Button> 
     
        <Button  style={{ marginLeft: "auto" }}  disabled={isDisabled} variant="contained" color="primary" onClick={()=>handleSubmitRequest()}>
            Submit
        </Button>    
        </>
      }
        ...
        <InputLabel id="demo-simple-select-filled-label">Office</InputLabel>

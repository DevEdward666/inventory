...
import {
  ...
  action_set_search_item,
} from "../../Services/Actions/Inventory_Actions";
import {action_set_open_backdrop,action_generate_report} from '../../Services/Actions/Defaults_Actions'
...
const GRtable = (props) => {
      ...
      if (mounted) {
        if (
          ...
          singlerequestdetails?.data.map((item) => {
            ...
            settotalrequest((prev) => prev + item?.costamount);
    ...
    if (arrayitem.length <= 0) {
      await setarrayitem((prevArray) => [
        ...prevArray,
        {
          ...
          reqqty: itemqty,
          costamount:
            parseInt(itemqty) * parseInt(listarrayitem[0]?.averagecost),
          unitdesc: listarrayitem[0]?.unitdesc,
          packdesc: listarrayitem[0]?.packdesc,
  ...
  const handleSearch = useCallback(
    (event) => {
      dispatch(action_set_search_item(event.target.value));
    },
    [dispatch]
  );
  ...
  const createData = (
    ...
    return { stockcode, stockdesc, unitdesc, packdesc, averagecost };
  };
      ...
      <CardContent style={{ padding: 50 }}>
              ...
              {arrayitem.map((row, index) => {
                return (
                  <TableRow key={row.stockcode}>
                    ...
                    <TableCell>{row.costamount}</TableCell>
        ...
        <div style={{ display: "flex",marginTop:20 }}>
          ...
          onClick={() => {
            ...
            dispatch(action_generate_report(requestinfo?.data?.reqno));
          }}
        >
          Print Report
        </Button> 
